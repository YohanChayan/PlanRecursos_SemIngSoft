<x-app-layout>
   ...
   @if(isset($employee))
    ...
    <div class="relative pt-4">
      ...
      <input required autofocus type="password" minlength="5" value="{{ $employee->password ?? '' }}" <?php echo ($IsNew) ? 'required':'hidden'?>  id="password" name="password" class="w-full px-4 py-2 mt-2 mr-4 text-base text-black transition duration-500 ease-in-out transform rounded-lg bg-gray-100 focus:border-blueGray-500 focus:bg-white focus:outline-none focus:shadow-outline focus:ring-2 ring-offset-current ring-offset-2">
    </div>
