<x-app-layout>
    ...
    <div style="padding:15px;">
		...
		<a href="{{{ route ('adminR.chiefs.create')}}}">Agregar nuevo Jefe de proyecto</a>
			</button>
    </div>
    ...
    <table class="min-w-full border-collapse block md:table">
		...
		@foreach($chiefs as $chief )
			...
			@endif
				...
				<td class="p-2 md:border md:border-grey-500 text-left block md:table-cell"><span class="inline-block w-1/3 md:hidden font-bold">Correo</span>{{$chief->email}}</td>
				<td class="p-2 md:border md:border-grey-500 text-left block md:table-cell"><span class="inline-block w-1/3 md:hidden font-bold">Numero de telefono</span>{{$chief->phone_number}}</td>
				<td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">
					<span class="inline-block w-1/3 md:hidden font-bold">Actions</span>
					<button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 border border-blue-500 rounded">
					<a href="{{route('adminR.chiefs.edit', $chief)}}">
						Editar
					</a>	
					
					</button>
					<button class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 border border-red-500 rounded">
					<form action="{{ route('adminR.chiefs.destroy',$chief) }}" method="POST">
							@csrf
							@method('DELETE')
